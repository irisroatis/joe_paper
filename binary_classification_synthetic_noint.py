...
from sklearn.metrics import accuracy_score, roc_auc_score, f1_score, log_loss, mean_squared_error, mean_absolute_error, r2_score, brier_score_loss
...
for i in range(50):
    ...
    for nr_bins_x1 in list_nr_bins:
        ...
        dictionary_performance_continuous['brier'].append(brier_score_loss(df_test_cont['y'], y_pred_proba_cont)) # Added Brier Score
        ...
        dictionary_performance_nocat['brier'].append(brier_score_loss(df_test['y'], y_pred_proba_nocat)) # Added Brier Score
        ...
        dictionary_performance[nr_bins_x1]['brier'].append(brier_score_loss(y_test_target, y_pred_proba)) # Added Brier Score
        ...
        dictionary_performance_glmm[nr_bins_x1]['brier'].append(brier_score_loss(y_test_glmm, y_pred_proba_glmm)) # Added Brier Score
        ...
        dictionary_performance_tarreg[nr_bins_x1]['brier'].append(brier_score_loss(y_test_tarreg, y_pred_proba_tarreg)) # Added Brier Score
        ...
        dictionary_performance_ordinal[nr_bins_x1]['brier'].append(brier_score_loss(y_test_oe, y_pred_proba_oe)) # Added Brier Score
        ...
        dictionary_performance_onehot[nr_bins_x1]['brier'].append(brier_score_loss(y_test_onehot, y_pred_proba_onehot)) # Added Brier Score
        ...
        for combination in [ [[3,1],'sigmoid'], [[1],'linear']]:
            ...
            for k in categorical_variables:
                ...
                # inputs[k] = Input(shape=(4,), name = str(k))
            ...
            for cat in categorical_variables:
                ...
                for idx in range(select_dataset.shape[0]):
                    ...
                    # Ensure output is treated as a single value
...
for performance_metric in ['accuracy', 'auc', 'f1', 'logloss', 'brier']: 
    ...
    y_offset = 0.05 if performance_metric == 'brier' else 0.95


    for center, nr_bins in zip(bin_centers[:-2], list_nr_bins):
        plt.text(center + 0.5, y_max * y_offset, f'{nr_bins} cat', ha='center', va='bottom', fontsize=10, weight='bold') # Adjusted y offset for logloss

    plt.text(bin_centers[-1], y_max * y_offset, 'Extra', ha='center', va='bottom', fontsize=10, weight='bold') # Adjusted y offset
